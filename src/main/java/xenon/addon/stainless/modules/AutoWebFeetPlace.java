...
public class AutoWebFeetPlace extends StainlessModule {
    ...
    private boolean checkPacing() {
        double aps = Math.max(0.5, attemptsPerSecond.get());
        long intervalNs = (long) (1_000_000_000L / aps);
        long now = System.nanoTime();
        if (now - lastAttemptNs < intervalNs) return false;
        lastAttemptNs = now;
        return true;
    }

    private void manageSurroundLock(PlayerEntity target) {
    ...
    private FindItemResult ensureItem() {
