...
public class StainlessModule extends Module {
    ...
    public void sendDisableMsg(String reason) {
        ...
        // Module name colored red, then reason in gray
        Text msg = Text.empty()
            .append(prefix())
            .append(Text.literal(name).formatted(OFF_COLOR))
    ...
    private void sendToggleMsg(boolean enabled) {
        if (!shouldChat()) return;
        ChatUtils.forceNextPrefixClass(getClass());
        // Only show prefix + module name; color the module name green when enabled, red when disabled
        Text msg = Text.empty()
            .append(prefix())
            .append(Text.literal(name).formatted(enabled ? ON_COLOR : OFF_COLOR));
